...
SEPARATE `kind: kubernetes` profile per bed that deploys it (`check-k3s-vm-ctx`,
...
- **`charly check live <name>`** — evaluates a **running deployment**
  (pod / vm / host / kubernetes; auto-resolved by `<name>` against
