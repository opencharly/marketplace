...
name: migrate
description: |-
  The `charly migrate` command — the single idempotent migration bringing any opencharly config to the latest schema CalVer. Covers the CalVer stamp, the CUE-anchored HEAD/floor, the declarative migration table, the LatestSchemaVersion() gate, or adding a schema cutover.
---
...
2. Run the spec repo's **`charly task cue-gen`** to regenerate `spec/spec/version_gen.go`.
...
**The validated schema is CUE-single-source.** The `@go()`-annotated `spec/schema/*.cue` defs are the sole source for the Go param structs (generated into `spec/spec` by `charly task cue-gen`); a wire-key change is a CUE edit first, then `charly task cue-gen`, then — only if it breaks existing on-disk configs — a migration-table entry here (plus the `#SchemaVersion` bump above). A pure codegen refactor that leaves every authored wire key untouched needs NO migration entry and NO `version:` bump. See the `/charly-internals:go` recipe "How to change the charly.yml schema (CUE is the single source of truth)".
