...
- **Ingress** (`charly/cue_schema.go`): validates the INPUT config a user authors
  ...
  `task cue:gen` (`cue exp gengotypes`, run in the spec module — vendored in-repo; the
  superproject task chains it), kept honest by the reproducibility + parity tests. Owned by `/charly-build:validate`; the schema-change codegen
