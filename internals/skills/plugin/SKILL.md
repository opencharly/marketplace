...
- `go test ./...` — the registry/transport/schema seams (`TestPluginGRPCRoundTrip`,
  ...
  `TestBuiltinPluginSchemasSplice` asserts that every builtin schema splices — like `charly marketplace drift`, it
    runs in no CI workflow and is red only for whoever runs it).
