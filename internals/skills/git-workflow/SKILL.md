...
- **`git add` is all-or-nothing: never name a path that no longer exists.** A single `git add` whose pathspec includes a vanished path fails the WHOLE add — `fatal: pathspec '<old>' did not match any files`, exit 128, nothing staged, including the paths that did resolve. Two shapes hit this routinely, and both are silent because a PRIOR command already staged something:
  - **A deletion**, where the removed file's path is still named in the `add`. Stage by DIRECTORY or with `git add -u` instead.
  - **A rename plus a content edit**, where `git mv old new` is followed by `git add old new`. The `git mv` already staged the rename, so the failed add leaves `git status` looking exactly as expected (`RM old -> new`) and the subsequent `git commit` succeeds — landing the rename with the content edit dropped.

  **The rename shape defeats the check above**, which is why it needs its own rule: `git show --stat` DOES list the intended file, as `old.md => new.md | 0`. Presence in the list is not the signal — the tells are the **magnitude** (`| 0`, zero insertions, on a commit that was supposed to change content) and a **leftover ` M <new>`** in `git status` afterwards. Read the numbers and the post-commit status, not just the filenames. Stage a rename+edit as `git add <new>` alone (or `git add -u`), and re-read `git status --short` after every `git add` whose exit code you did not check.
