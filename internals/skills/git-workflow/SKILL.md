...
- **Atomic on `main`, never on `feat/`.** The auto-merge workflow's `--squash` folds the author's change, any review-round fix commits, and the merge-time CalVer rewrite (see "CalVer" in `references/validator-and-calver.md`) into one commit whose message is composed by the validator and carried through the merge; the `feat/` branch may freely accumulate fix commits across review rounds. Two separate cutovers must never share one PR.
