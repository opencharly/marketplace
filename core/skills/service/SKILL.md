...
- The required dependency layer is INJECTED, not policed: a box composing a `service:` candy has the active init's `depends_candy` (from the embedded `init:` vocabulary) added to its composition automatically, so a box never has to list `supervisord` by hand and can no longer be missing it. The `charly box validate` presence check that used to enforce this is gone — it could not catch the shape that actually broke deploys
